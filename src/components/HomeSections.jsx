      ...
      <section style={{ padding: '24px 0' }}>
        <div style={{ maxWidth: 980, margin: '0 auto' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div ref={sliderRef} onScroll={handleCityScroll} className="snap-container" style={{ display: 'flex', gap: 12, paddingBottom: 6 }}>
              {AIRPORTS.map((a) => (
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 10, gap: 6 }}>
              {Array.from({ length: cityPages }).map((_, i) => (
                <div key={i} onClick={() => { const el = sliderRef.current; if (el) el.scrollTo({ left: i * el.clientWidth, behavior: 'smooth' }); }} style={{ width: 8, height: 8, borderRadius: 999, background: i === cityPage ? 'var(--accent)' : 'rgba(0,0,0,0.12)', cursor: 'pointer' }} />
              ))}
            </div>
          </div>
        </div>
